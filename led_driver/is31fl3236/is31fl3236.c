...
SemaphoreHandle_t stm8_led_mux ;
...
static esp_err_t stm8_master_write_slave( uint8_t data)
{
    xSemaphoreTake(stm8_led_mux, portMAX_DELAY);
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (0x28 << 1) | 0, 1);
    i2c_master_write_byte(cmd, data, 1);
    i2c_master_stop(cmd);
    esp_err_t ret = i2c_master_cmd_begin(0, cmd, 1000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    xSemaphoreGive(stm8_led_mux);
...
esp_err_t stm8_led_init() 
{
    vSemaphoreCreateBinary(stm8_led_mux);
    if (stm8_led_mux == NULL) {
        ESP_LOGE(LED_TAG, "Could not create semaphore");
        return ESP_FAIL;
    }
     return ESP_OK;
...
void stm8_led_set_pwm(const uint32_t * va_led_v)
{
    int i;
    uint8_t red_led_val, blue_led_val, green_led_val;
    
    blue_led_val  = 0xFF & (va_led_v[0]);
    green_led_val = 0xFF & (va_led_v[0] >> 8);
    red_led_val   = 0xFF & (va_led_v[0] >> 16);
    red_led_val = ~red_led_val;
    green_led_val = ~green_led_val;
    blue_led_val = ~blue_led_val;

    stm8_master_write_slave('$');
    stm8_master_write_slave(red_led_val);
    stm8_master_write_slave(green_led_val);
    stm8_master_write_slave(blue_led_val); 
    stm8_master_write_slave('#');   
}
//-----------------------------------------------------------------------------------------------------